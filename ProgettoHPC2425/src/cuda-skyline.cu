 ...
 * Operatore Skyline CUDA versione principale, 1.
 * Luca Venturi
 ...
 * Praticamente ogni thread esegue il ciclo for interno della versione seriale.
 */
__global__ void kernel_skyline(const float *P, int *s, int N, int D)
{
    ...
    if (i < N && s[i])
    {
        ...
        for (int j = 0; j < N; j++)
        {
            ...
            {
                /* Rimuove il punto j-esimo dallo skyline, si usa un'operazione atomica per evitare contese. */
                atomicExch(&s[j], 0);
            }
        }
    }
...
/**
 * Prepara i dati e lancia il kernel per il calcolo dello skyline.
...
int skyline(const points_t *points, int *s)
{
    ...
    */
    ...
    /* Lancia il kernel per il calcolo dello skyline con NBLOCKS blocchi di BLKDIM thread ciascuno. */
    kernel_skyline<<<NBLOCKS, BLKDIM>>>(d_P, d_s, N, D);

    /* Copia l'array s dalla GPU alla CPU. */
