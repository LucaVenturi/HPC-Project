...
__global__ void kernel_skyline(const float *P, int *s, const int N, const int D, int *r, const int i)
{
    const int j = threadIdx.x + blockIdx.x * blockDim.x;

    if ( j < N && s[j] && dominates( &(P[i*D]), &(P[j*D]), D ) ) {
        s[j] = 0;
        atomicSub(r, 1);
    }
}
...
int skyline(const points_t *points, int *s)
{
    ...
    for (int i=0; i<N; i++) {
        if ( s[i] ) {
            ...
            kernel_skyline<<<NBLOCKS, BLKDIM>>>(d_P, d_s, N, D, d_r, i);
