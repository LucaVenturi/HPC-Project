...
int skyline( const points_t *points, int *s )
{
    ...
    #pragma omp parallel
    {
        /*
            ...
            Si usa un ciclo for parallelo per velocizzare l'inizializzazione.
        */
        #pragma omp for
        for (int i=0; i<N; i++) {
            s[i] = 1;
        ...
        // Non serve una barriera esplicita in quanto
        // #pragma omp for ne crea una implicita.
